...
class CrochetLink {
  ...
  get defLen() {
    return this._defLen;
  } // link lenght for ForceLayout simulation
  ...
  } // should it be drawn in documents
  ...
  getNeighborhood(withEnds = true) {
    let nodes = this.source
      .getNeighborNodes()
      .concat(this.target.getNeighborNodes());
    if (withEnds) {
      nodes = nodes.concat(this.target).concat(this.source);
    }
    return nodes;
  }
  ...
  replaceNode(oldNode, newNode) {
    if (newNode === this.source || newNode === this.target)
      throw new Error("crocherLink.replaceNode : new node not distinct");
    if (oldNode !== this.source && oldNode !== this.target)
      throw new Error("crocherLink.replaceNode : old node not from link");
    if (oldNode._context !== newNode._context)
      throw new Error(
        "crocherLink.replaceNode : both nodes must be fro msmae Stitch"
      );
