class FactoryClass {
  constructor(exampleClass, attributeName = false, numAddArguments = 0) {
    ...
    if (attributeName !== false) {
      if (exampleClass.prototype[attributeName]) {
        ...
        throw new Error(
          `factoryClass : class '${exampleClass.name}' does not have the attribute/method '${attributeName}'`
        );
      }
    } else {
  ...
  registerClass(newClass, classKey = false, additionalArguments = []) {
    ...
    if (this.getKeyAttr !== false) {
      // i.e. class has a "code accessor" - get it
      try {
        key =
        ...
        throw new Error(
          `${this.className} factory : class ${newClass.name} must expose the '${this.getTypeFunc}' method or attribute`
        );
      }
    } else {
      // class has no code accessor - ge it explicitly
      if (classKey === false)
        throw new Error(
          `factoryClass : factory of '${this.className}' needs explicit keys`
        );
      key = classKey;
    }
    const item = {
  ...
  getNewObject(type, ...rest) {
    ...
    if (typeof Cls === "undefined")
      throw new Error(
        `${this.className} factory : invalid ${this.className} type '${type}'`
      );
