...
class TransformationTwoVector {
  constructor() {
    ...
    if (clean.length < 2)
      throw new Error("TransformationTwoVector : not enough vectors");
    this.vAbs = clean[0];
    this.vPerc = clean[1];
...
class GraphCommand {
  ...
  constructor(cmd, params) {
    if (typeof cmd !== "string")
      throw new Error("graphCommand : command must be a string");
    if (!(params instanceof Array))
      throw new Error(
        "graphCommand : params must be an Array of TransformationTwoVector"
      );
...
function tokenizeDrawingCommands(pathStr = "") {
  ...
  const tokenized = commands
    ...
    .filter(
      (e) => Plotter.allowedCommands.get(e.cmd).numPts <= e.params.length
    ); // with enough param points
  return tokenized;
}
...
function parseDrawingCmds(tokenizedCommands = []) {
  ...
  const parsedCmds = tokenizedCommands.map((token) => {
    const paramTwoVectors = token.params.map((param) => {
      // "a,a" | "p,p%" | "p,p % a,a"
      ...
      const twoVecArray = param
        ...
        .map((g) => g.split(",").map((h) => parseFloat(h))) // [[ax,ay],[px,py]] etc..
...
class Plotter {
  ...
  static SVGpathGeneric(cmd, numPts, vecArr) {
    if (vecArr.filter((e) => e instanceof Vec2d).length < numPts) {
      throw new Error(
        `getSVGcmd : not enough vectors in veec2Arr for command '${cmd}'. Expected ${numPts}`
      );
    }
  ...
  static SVGpathArc(vecArr, base, params) {
    if (vecArr.filter((e) => e instanceof Vec2d).length < 4) {
      throw new Error(
        "getSVGcmd : not enough vectors in vecArr for command 'a'. Expected 4"
      );
    }
    let dCmd = "a ";
    ...
    dCmd += params[2].vAbs._x + "," + params[2].vAbs._y + ","; // sweep and arc flags - expected to be 0 or 1
  ...
  static SVGpathCommandFactory(func, cmd, params, numPts, base, vecArr) {
    switch (func) {
      case "reset":
        return Plotter.SVGpathResetPen(base);
      case "arc":
        return Plotter.SVGpathArc(vecArr, base, params);
      case "generic":
        return Plotter.SVGpathGeneric(cmd, numPts, vecArr);
      default:
        return "";
    }
  }
  ...
  static getPathTxt(sourceVec, targetVec, pathDef) {
    if (!(sourceVec instanceof Vec2d))
      throw new Error("getPathTxt : sourceVec must be a Vec2d vector");
    if (!(targetVec instanceof Vec2d))
      throw new Error("getPathTxt : targetVec must be a Vec2d vector");
    if (!(pathDef instanceof Array))
      throw new Error("pathDef : targetVec must be an Array of [graphCommand]");
    ...
    const pathSteps = pathDef.map((pathCommand) => {
      const vectors = pathCommand.params.map((e) => e.calcPathPoint(base));
      const { numPts, mappedCmd, funcs } = Plotter.allowedCommands.get(
        pathCommand.cmd
      );
      const pathStepParts = funcs.map((e) =>
        Plotter.SVGpathCommandFactory(
          e,
          mappedCmd, // 'a'
          pathCommand.params,
          numPts, // 4
