...
class CrochetNode {
  ...
  get isLoopable() {
    return this._isLoopable;
  } // can the node serve as a "loop" node other stitches conenct to

  get color() {
    return this._color;
  } // how to draw the node
  ...
  constructor(
    ...
    if (arguments.length < 5)
      throw new Error("CrochetNode : not enough arguments");
    ...
    if (argCoordinates instanceof Array) {
      if (argCoordinates.length < 2)
        throw new Error(
          "CrochetNode : there must be two coordinatess in coordinates Array"
        );
      if (
        typeof argCoordinates[0] !== "number" ||
        typeof argCoordinates[1] !== "number"
      )
        throw new Error("CrochetNode : both coordinates must be numbers");
      ...
      throw new Error(
        "CrochetNode : coordinates must be an Array or Vec2d vector"
      );
    }
  ...
  toString() {
    return `[${this.id} (${this.type}) : ${this.y.toFixed(1)}; ${this.x.toFixed(
      1
    )}]`;
  }
  ...
  getNeighborLinks(dir = "", type = "") {
    ...
    if (dir !== "" && dir !== "in" && dir !== "out")
      throw new Error("crochetNode.getNeighborLinks: invalid direction");
  ...
  getNeighborNodes(dir = "", type = "") {
    // The filtering link direction must be either empty, "IN" or "OUT"
    if (dir !== "" && dir !== "in" && dir !== "out")
      throw new Error("crochetNode.getNeighborNodes: invalid direction");
