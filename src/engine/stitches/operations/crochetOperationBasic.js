...
class CrochetOperationBasic extends CrochetOperation {
  ...
  static CALC_DEF_NEW_POS(fromNode, len, toNode, lastNeedleNode) {
    ...
    if (goTowardsNeedle) {
      ...
      const neighborVecs = neighborNodes
        ...
        .map((e) => startVector.sub(e.getVector()).unit());
  ...
  exec(subject) {
    const [newLinkType, newNodeType, newLinkLength = false, drawPathName = ""] =
      this.params;
    ...
    const newNode = CrochetOperation.nodeFactory.getNewObject(
      ...
      sourceNode.getVector()
    );
    const newLink = CrochetOperation.linkFactory.getNewObject(
      newLinkType,
      subject.contextStitch,
      sourceNode,
      newNode
    ...
    const linkLength = newLinkLength !== false ? newLinkLength : newLink.defLen;

    const newPos = CrochetOperationBasic.CALC_DEF_NEW_POS(
      sourceNode,
      linkLength,
      newNode,
      activeNode
    ...
    if (newLink.isPrintable && drawPathName !== "")
      newLink.selectNewPath(drawPathName);
