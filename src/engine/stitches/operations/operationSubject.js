...
class OperationSubject {
  constructor(needle = [], stitch, otherLoops = []) {
    if (!(needle instanceof Array))
      throw new Error(
        "crochet operationSubject : needle must be an Array [crochetNode]"
      );
    if (!(otherLoops instanceof Array))
      throw new Error(
        "crochet operationSubject : otherLoops must be an Array [crochetNode]"
      );
