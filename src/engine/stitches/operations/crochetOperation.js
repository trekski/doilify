...
class CrochetOperation {
  ...
  constructor(cmds) {
    if (!(cmds instanceof Array))
      throw new Error("crochetOperation : params must be an array [string]");
    this.params = cmds.filter((e) => typeof e === "string"); // first argument will be an Array of the operation parameters
    if (this.params.length < this.minParams)
      throw new Error(
        `crochetOperation : not enough parameters for '${this.commandName}'. Expected : ${this.minParams}`
      );
  }
  ...
  exec(subject) {
    if (!(subject instanceof OperationSubject))
      throw new Error("crochetOperation : needs a valid operationSubject");
