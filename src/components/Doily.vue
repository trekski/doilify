...
import StitchSprite from "./Stitch.vue";
...
export default {
  ...
  data() {
    return {
      ...
      simulation: d3
        ...
        .force(
          "charge",
          d3.forceManyBody().strength(-0.2).distanceMin(5).distanceMax(50)
        ),
      live_node: Object,
      selected_nodes: [],
      simulationLive: true,
  ...
  computed: {
    all_nodes() {
      let n = [];
      this.stitches.reduce((prevStitch, currStitch) => {
    ...
    unselected_nodes() {
      return this.selectable_nodes.filter(
        (e) => !this.selected_nodes.includes(e)
      );
    },
    requiredLoops() {
  ...
  methods: {
    ...
    refresh_simulation() {
      ...
      this.simulation
        ...
        .force(
          "link",
          d3.forceLink(this.all_links).distance((d) => d.getLen())
        )
        .alpha(1)
        .restart();
    },
    ...
    makeStitch() {
      ...
      s.crochet(this.live_node, this.selected_nodes);
      this.stitches.push(s);
      this.live_node = s.getLastLoop();
      this.refresh_simulation();
    ...
    autoSelectLoops() {
      ...
      let nextNewNode =
        lastUserLazyLoop !== undefined ? lastUserLazyLoop : lastUsedLoop;
      const newNodes = lastUserLazyLoop !== undefined ? [lastUserLazyLoop] : [];
      nextNewNode = nextNewNode.getSeqLoop("next");
      while (
        nextNewNode !== undefined &&
        newNodes.length < this.requiredLoops
      ) {
    ...
    orderSelectedNodes() {
      ...
      orderedNodes.sort((firstNode, secondNode) => {
